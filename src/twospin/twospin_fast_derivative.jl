...
struct ModelDerivative{TH,Tis,Tic,Tid} <: AbstractModel
    # problem size
    n::Int
    m::Int
    # problem
...
function ModelDerivative(M_, Md_, V_, Hs, time_optimal)
    # problem size
    control_count = 1
    ...
    Tid = typeof(dt_idx)
    return ModelDerivative{TH,Tis,Tic,Tid}(n, m, Hs, time_optimal, state1_idx, state2_idx,
                                 state3_idx, state4_idx, intcontrols_idx, controls_idx,
                                 dcontrols_idx, dstate1_idx, d2controls_idx, dt_idx)
end

@inline Base.size(model::ModelDerivative) = model.n, model.m
...
# dynamics
function Altro.discrete_dynamics(::Type{EXP}, model::ModelDerivative,
...
function run_traj(;gate_type=iswap, evolution_time=50., dt=DT_PREF, verbose=true,
    ...
    model = ModelDerivative(M, Md, V, Hs, time_optimal)
    n, m = size(model)
    t0 = 0.
    dt_max = 2 * dt
    dt_min = dt / 1e1
    ...
    if isnothing(initial_pulse)
        ...
        # initial_dict = read_save(initial_pulse)
        # d2_
        # U0_ = initial_dict["astates"]
        U0_, ts = grab_controls(initial_pulse)
        dcontrols = [(U0_[k] - U0_[k+1]) / U0_[k] for k = 3:N-2]
        append!(dcontrols, 0.0)
        prepend!(dcontrols, 0.0)
        prepend!(dcontrols, 0.0)
        d2controls = [(dcontrols[k] - dcontrols[k+1]) / dcontrols[k] for k = 3:N-2]
        append!(d2controls, 0.0)
        prepend!(d2controls, 0.0)
        prepend!(d2controls, 0.0)
        U0 = V(vec([[u0_elem] for u0_elem in d2controls]))
    end
    for k = 1:N-1
